...
export const testLoginFlow = async () => {
  ...
  console.log('Initial state:', (store.getState() as { auth: unknown }).auth);
  ...
  try {
    store.dispatch(authLogin({ username: testCredentials.email, password: testCredentials.password }));

    const unsubscribe = store.subscribe(() => {
      const state = (store.getState() as unknown as { auth: { isLoading?: boolean; isError?: boolean; error?: string; data?: { token?: string } } }).auth;
...
export const loginChecklist = {
  api: {
    'API function exists': 'src/app/api/auth.tsx',
